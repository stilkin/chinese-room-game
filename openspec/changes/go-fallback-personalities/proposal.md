...
- **Default**: **Star-point** (post-gate). The original paper plan was Hugger; the round-robin gate at 50 games/direction showed Hugger losing 0/100 to Chaotic — overconcentrated dumpling shapes lose to spread-out random play because the latter accidentally claims more territory. Star-point lands at slider position 2 (mid) with sensible textbook-Go opening behaviour.
- **Migrate** persisted config: any stored value not in the Go user-facing set (including legacy `pileFocus`, `ownPileAdjacent`, `greedyConnect`, `greedyConnectDefense`, the prior CF defaults) is silently mapped to `goStarPoints` on read. (Already mostly handled by the Go cutover's coercion to `random`; this change tightens the default to `goStarPoints`.)
